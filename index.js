    ...
    try {
        ...
        usersToInit.forEach(email => {
            ...
            const merged = periodizacaoTemplate.map(newItem => {
                const old = existing.find((o) => o.id === newItem.id);
...
function renderTrainingHistory(email) {
    ...
    monthlyHistory.forEach((item, idx) => {
...
window.openHistoryDetail = (timestamp) => {
    ...
    const item = db.completedWorkouts?.[email]?.find((x) => x.timestamp === timestamp || x.date === timestamp /* fallback */);
...
function loadTrainingScreen(type, email) {
    ...
    if (listContainer) {
        ...
        const renderedItems = plan.map((ex, i, arr) => {
        ...
        renderedItems.forEach((el) => listContainer.appendChild(el));
...
window.openFinishWorkoutModal = (type, extraData = null) => {
    ...
    const imgPreview = document.getElementById('finish-photo-preview');
    const placeholder = document.getElementById('finish-photo-placeholder');
    const input = document.getElementById('workout-photo-input');
...
function loadOutdoorTrackingScreen(activity) {
...
function stopOutdoorTracking() {
    ...
    window.openFinishWorkoutModal(currentActivityType, extraData);
}

// --- PERIODIZATION LOGIC ---
window.togglePeriodizationStatus = (id) => {
    const userEmail = getCurrentUser();
    if (!userEmail) return;
    const db = getDatabase();
    const periodizacao = db.trainingPlans.periodizacao[userEmail];
    if (!periodizacao) return;
    const item = periodizacao.find((p) => p.id === id);
    if (item) {
...
window.openPeriodizationModal = (id) => {
    const userEmail = getCurrentUser();
    if (!userEmail) return;
    const db = getDatabase();
    const periodizacao = db.trainingPlans.periodizacao[userEmail];
...
function loadPeriodizationScreen() {
    ...
    if (container) {
        ...
        if (periodizacao.length === 0) {
            ...
            sortedPeriodization.forEach((p, index) => {
                ...
                if (!isLocked) card.onclick = () => window.openPeriodizationModal(p.id);
                let actionButton = isLocked ? `
                    ...
                    ` : `
                        <button onclick="event.stopPropagation(); window.togglePeriodizationStatus(${p.id})" class="text-xs font-bold px-3 py-1.5 rounded-full border ${statusBg} ${statusColor} flex items-center gap-1.5 hover:brightness-110 active:scale-95 transition-all shadow-md z-10">
    ...
    if (loginForm) {
        loginForm.addEventListener('submit', (e) => {
            ...
            if (loginBtn) {
                ...
                setTimeout(() => {
                    const db = getDatabase();
                    const userExists = db.users.find((u) => u.email.toLowerCase() === email);
...
function setupEventListeners() {
    ...
    window.openExerciseModal = (idx, type) => {
        const db = getDatabase();
        const email = getCurrentUser();
        const exercise = db.trainingPlans[`treinos${type}`][email][idx];
        document.getElementById('modal-exercise-name').textContent = exercise.name;
        document.getElementById('modal-exercise-img').src = exercise.img;
...
// Global functions exports for HTML handlers
